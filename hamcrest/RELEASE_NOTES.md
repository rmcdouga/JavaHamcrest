...
* Create custom matchers more succinctly using `matcher()` static factory methods on `FeatureMatcher` and `TypeSafeDiagnosingMatcher`
...
* New `IsUnmodifiable.isUnmodifiableCollection` matcher for testing if a collection is unmodifiable.
* New matchers for testing the content of files in `org.hamcrest.io.FileMatchers` and `org.hamcrest.io.PathMatchers`
* Various bug fixes